...
class SolverImpl
{
	...
	typedef _KIWI_MAP_TYPE<Variable, Symbol> VarMap;

	typedef _KIWI_MAP_TYPE<Symbol, Row*> RowMap;

	typedef _KIWI_MAP_TYPE<Constraint, Tag> CnMap;

	typedef _KIWI_MAP_TYPE<Variable, EditInfo> EditMap;
...
public:
	...
	*/
	void addConstraint( const Constraint& constraint )
	{
		...
		Tag tag;
		_KIWI_UNIQUE_PTR<Row> rowptr( createRow( constraint, tag ) );
	...
	*/
	void removeConstraint( const Constraint& constraint )
	{
		...
		{
			_KIWI_UNIQUE_PTR<Row> rowptr( row_it->second );
			m_rows.erase( row_it );
		}
		else
		{
...
private:
	...
	*/
 	...
 	*/
 	bool addWithArtificialVariable( const Row& row )
 	{
		...
		if( it != m_rows.end() )
		{
			_KIWI_UNIQUE_PTR<Row> rowptr( it->second );
			m_rows.erase( it );
	...
	*/
	...
	_KIWI_UNIQUE_PTR<Row> m_objective;
	_KIWI_UNIQUE_PTR<Row> m_artificial;
	Symbol::Id m_id_tick;
};
